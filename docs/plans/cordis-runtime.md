...
- [x] **2k — Missing deps.** Audited every `@deepseek-ai/dsh-*` import in `electron/cordis/*` against `package.json`: **all declared** (incl. the 2h/2e/2g additions `dsh-token-meter`, `dsh-compaction-basic`, `dsh-llm-retry`, `dsh-user-approval`, `dsh-user-questions`). `dsh-attachment` is **not used anywhere**, so it's intentionally NOT added (add only when relied upon). `npm run compile` builds all four bundles cleanly with the new deps; `bundle-guard.test.ts` (12 tests) green — every node builtin the new deps pull in (`node:timers/promises`, etc.) is already in `NODE_BUILTINS`. win32-arm64 packaging remains a release/CI verification (can't exercise on darwin) but introduces no new native addon beyond better-sqlite3/onnxruntime, which are already arch-handled.

...
**Next concrete step (2j):** sandbox/cwd. The sandbox stack is already mounted in `mountCodingStack` (`dsh-sandbox-local` → `dsh-sandbox-policy` `{mode, workspaceRoot:cwd}` → `dsh-fs-sandbox` `{cwd}` → `dsh-fs-observation-policy`), currently `mode:"danger-full-access"`. 2j is a verification + tightening pass: (1) confirm fs/bash traversal is actually restricted to `cwd` (write outside cwd is denied) with a live test; (2) preserve Cairn's automation-dev restriction (automation runs must be file-only-no-shell) — either via a tighter `sandboxMode` (`workspace-write`) for the automation caller or by keeping the plan-mode-style read-only gate; (3) thread a `sandboxMode` option through `runCordisCodingLoop` so the wiring step can pick the right preset per caller. Then 2k deps (`dsh-attachment`, re-verify guards + win32-arm64). Then the FINAL integration: wire `runCordisCodingLoop` into `electron/ipc/pi-agent.ts` behind `CAIRN_ENGINE` — `pi-agent:prompt`/`approve-plan` call the loop; `respond-tool`/`respond-questions`/`respond-doom-loop`/`set-mode` resolve the per-turn pending maps; add manual `/compact` via `ctx.compaction.compactNow(agent)` on the live handle; `sessions`/`runningLoops`/`is-running` stay Cairn-side (builtin path untouched). Only then flip the coding default. Loop = `electron/cordis/run-cordis-coding.ts`; plugins = `cairn-plugins.ts` (cairnCoding/PlanMode/Approval/DoomLoop/Questions); shared services in `getContext` (tokenMeter/compaction/llm-retry); skills via `registerSkillTool`+`renderSkillsXml`; proofs = `coding-agent.live.test.ts` (5 live) + `doom-loop.test.ts` + `compaction-retry.test.ts` (unit).

...
**Next concrete step (2f):** doom-loop guard. No dsh package — reimplement Cairn's builtin logic (`electron/lib/pi-agent-loop.ts`: rolling `recentToolCalls` window + `toolCallSignature` (canonicalized name+args) + `DOOM_LOOP_THRESHOLD=3`) as a `tools/pre-execute` guard plugin (`cairnDoomLoopPlugin`), bridging `pi-agent:doom-loop` ⇄ `pi-agent:respond-doom-loop` (block until the user allows/denies; allow → run + stop re-pausing this session; deny → the tool is denied). Then 2h compaction/retries (`dsh-compaction-basic` / `dsh-llm-retry`), 2i skills (`cairn-skills`), 2j sandbox/cwd, 2k deps. Then the FINAL integration: wire `runCordisCodingLoop` into `electron/ipc/pi-agent.ts` behind `CAIRN_ENGINE` — `pi-agent:prompt`/`approve-plan` call the loop; `respond-tool`/`respond-questions`/`respond-doom-loop`/`set-mode` resolve the per-turn pending maps; `sessions`/`runningLoops`/`is-running` stay Cairn-side (builtin path untouched). Only then flip the coding default. Loop = `electron/cordis/run-cordis-coding.ts`; plugins = `cairn-plugins.ts` (cairnCodingPlugin/PlanMode/Approval/Questions); live proof = `electron/cordis/coding-agent.live.test.ts` (4 tests: turn / persistence / plan-mode / approval).

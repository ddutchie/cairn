...
> **Finding (2026-08-19): the heartbeat Cordis path is PARTIAL — connector/script/approval tools are NOT yet bridged.** The runner now calls `runCordisLoop` (`heartbeat-runner.ts:441`) passing only `onToken`/`onThought`/`onUsage`/`emitToolCall`/`emitToolCallDone`/`signal`. It does **not** pass: `extraTools` (the MCP/connector tools from `getExternalToolDefs` at `heartbeat-runner.ts:327`), `runScript`/`writeRunFile`/`deliverFile` (`heartbeat-runner.ts:240/264/271`), or `makeApprovalGate` (`heartbeat-runner.ts:38`). **Consequence:** a plain data-only automation (notes/tasks) works, but (a) a connector-aware automation runs WITHOUT its attached connector tools, and (b) a script automation (run_script/write_run_file/deliver_file) cannot run its scripts — both silently degrade. `runCordisLoop` (`run-cordis-loop.ts:45`) has no `extraTools`/approval/script slots yet. **This is the top Phase-3-adjacent follow-up**: add `extraTools` (external MCP defs), an approval seam, and the three script executors to the `runCordisLoop` options so heartbeat is fully functional on Cordis. Until then, heartbeat is "data-only works, connectors+scripts don't."
...
**Where we are:** Phase 1 (chat) DONE + shipping. **Phase 1.5 (coding agent) DONE** — 2a–2l + FINAL wiring complete. **Phase 2 COMPLETE** — 2a (deprecate) + 2b (port user-style + heartbeat to `runCordisLoop`) + 2c (collapse `CAIRN_ENGINE`, delete the 5 frozen loop files + dead `coding-tools/*`, retire 8 builtin-only tests) + 2d (prune `pi_agent_llm_history`) + 2e (one-shot `runOneShot` at `one-shot.ts:10`, 8/8 callers). `CAIRN_ENGINE` is fully removed from `electron/`. The frozen builtin loops (`chat-loop.ts`, `pi-agent-loop.ts`, `chat-subagent-loop.ts`, `compaction.ts`, `coding-tools/subagent.ts`) and `pi_agent_llm_history` are **deleted** (`90f0b960`, +208/−8232). Shared types moved to `electron/lib/pi-agent-types.ts`; doom-loop consts inlined into `cairn-plugins.ts`. Full electron suite 77 files/1106 (was 90/1178); live coding 8/8. **Next: Phase 3** (in-app user plugins + dsh client/UI — see §9 + the Phase 3 kickoff below).

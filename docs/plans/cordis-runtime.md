...
**Next concrete step (2d):** plan mode for the coding loop. Mount `dsh-plan-mode` (already part of `mountCodingStack` — its `exit_plan_mode` tool + PLAN_MODE_ALLOWED read-only gate) and bridge the `pi-agent` plan IPC: `set-mode`/`approve-plan`/`plan-note`/`mode-change`. Reuse Cairn's plan-note PRD prompt via `cairnSystemPromptPlugin`. Then 2e approvals (HITL via `dsh-user-approval` + `dsh-permission-presets`, bridge `tool-confirm-required`⇄`respond-tool`), 2f doom-loop, 2h compaction/retries, 2i skills, 2j sandbox, 2k deps. Wire `runCordisCodingLoop` into `electron/ipc/pi-agent.ts`'s `pi-agent:prompt`/`approve-plan` behind `CAIRN_ENGINE` (builtin untouched) and only then consider flipping the coding default. The bridge lives in `cairnCodingPlugin` (`electron/cordis/cairn-plugins.ts`) + `runCordisCodingLoop` (`electron/cordis/run-cordis-coding.ts`); the live proof is `electron/cordis/coding-agent.live.test.ts` (incl. jsonl multi-turn resume).

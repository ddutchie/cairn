...
**Next concrete step (2e):** approvals (HITL) for the coding loop. Mount `dsh-user-approval` + `dsh-permission-presets`, bridge dsh's approval seam ⇄ Cairn's `tool-confirm-required`/`respond-tool` IPC (+ grant session/command) with the `autoApprove` bypass. Cairn's approval flow: when `autoApprove` is off, dsh asks before a mutating tool → emit `pi-agent:tool-confirm-required` → block until `pi-agent:respond-tool`. Author the `automation-dev` file-only-no-shell preset. Then 2f doom-loop, 2h compaction/retries, 2i skills, 2j sandbox, 2k deps. Wire `runCordisCodingLoop` into `electron/ipc/pi-agent.ts`'s `pi-agent:prompt`/`approve-plan` behind `CAIRN_ENGINE` (builtin untouched), plus `set-mode`/`approve-plan`/`mode-change` for plan mode, and only then consider flipping the coding default. Loop: `runCordisCodingLoop` (`electron/cordis/run-cordis-coding.ts`); plugins in `cairn-plugins.ts` (cairnCodingPlugin, cairnPlanModePlugin); live proof `electron/cordis/coding-agent.live.test.ts` (turn/persistence/plan-mode).

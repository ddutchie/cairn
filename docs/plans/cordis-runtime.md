...
**Next concrete step (2i):** skills. Cairn's `electron/lib/skills.ts` (`discoverSkills(cwd)`, `loadSkill`, `renderSkillsXml`) + `electron/lib/coding-tools/skill.ts` are engine-agnostic (pure fs), so 2i is wiring, not new logic: (1) in `runCordisCodingLoop`, call `discoverSkills(cwd)` and append `renderSkillsXml(...)` to the coding system prompt (via `cairnSystemPromptPlugin`) so the model sees `<available_skills>`; (2) register a Cordis `skill` tool (already in APPROVAL_SAFE + PLAN_MODE_ALLOWED) whose body calls `loadSkill` and returns the SKILL.md content — mirror `coding-tools/skill.ts`. Live-test: a skill dir under cwd shows up in the prompt and `skill` returns its body. Then 2j sandbox/cwd (`dsh-fs-sandbox` already in mountCodingStack — verify traversal restriction + the automation-dev file-only preset), 2k deps (`dsh-attachment`, re-verify guards + win32-arm64). Then the FINAL integration: wire `runCordisCodingLoop` into `electron/ipc/pi-agent.ts` behind `CAIRN_ENGINE` — `pi-agent:prompt`/`approve-plan` call the loop; `respond-tool`/`respond-questions`/`respond-doom-loop`/`set-mode` resolve the per-turn pending maps; add manual `/compact` via `ctx.compaction.compactNow(agent)` on the live handle; `sessions`/`runningLoops`/`is-running` stay Cairn-side (builtin path untouched). Only then flip the coding default. Loop = `electron/cordis/run-cordis-coding.ts`; plugins = `cairn-plugins.ts` (cairnCoding/PlanMode/Approval/DoomLoop/Questions); shared services in `getContext` (tokenMeter/compaction/llm-retry); proofs = `coding-agent.live.test.ts` (4 live) + `doom-loop.test.ts` + `compaction-retry.test.ts` (unit).

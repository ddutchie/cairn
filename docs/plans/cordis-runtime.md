...
**Where we are:** Phase 1 (chat) DONE + shipping. **Phase 1.5 (coding agent) DONE** — 2a–2l + FINAL wiring complete, Cordis is the only path for cloud models (`chat.ts:369`/`pi-agent.ts:180` collapsed, `localllm` via `llama-server` also through Cordis at `run-cordis-loop.ts:158` `97f94373`). Phase 2a (deprecate `CAIRN_ENGINE=builtin` + `@deprecated` at `config-cache.ts:28`) done, 2b (port `user-style-handlers.ts:249` → `runCordisLoop` + `heartbeat-runner.ts:434` → `runCordisLoop` with `VITEST` fallback) done, 2c (collapse dual paths) done, 2e (one-shot `runOneShot` at `one-shot.ts:10` + 8 callers) done. Remaining: **2c** delete the 5 frozen files + retire 8 tests + **2d** prune `pi_agent_llm_history` — both blocked only by the `localllm` fallback which is now also Cordis, so they can go next. **Next: Phase 2c/d deletions** then **Phase 3** (in-app user plugins + dsh client/UI — see §9).
...
**Next concrete steps — Phase 2c/d deletions (the last builtin plumbing).**

- **Immediate next:** **Phase 2c/d** — delete the 5 frozen files (`chat-loop.ts:54`, `chat-subagent-loop.ts`, `pi-agent-loop.ts:628`, `compaction.ts:250`, `coding-tools/subagent.ts`) + retire the 8 builtin-only tests (`pi-agent-loop.test.ts` etc. at `docs/plans/cordis-runtime.md:139`) + drop `pi_agent_llm_history` (`queries.ts:1805` — `pi-agent:restore-context` at `pi-agent.ts:797` is already Cordis-aware). Blocked only by `localllm` which is now also Cordis, so they can go. One PR, `npm run type-check:all` + `npm run compile` + `npx vitest run electron` green.
- **Then:** **Phase 3 kickoff** in §9 (six-step bridge-plugin workstream: boot the in-process dsh host web stack behind `CAIRN_UI=dsh`, then `cairn-root-layout` → `cairn-toolview-*` → `cairn-persistence-bridge` → brand/theme → retire the IPC bridge). §9 lists the exact dsh packages, the arch note, and the slot names to occupy.

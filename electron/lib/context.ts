  ...
  return {
    ...
    tools: {
      read: [
        ...
        "get_task", "list_ready_tasks", "search_tasks",
      ...
      write: [
        "upsert_project",
        "ensure_note", "append_to_note", "patch_note",
    ...
    conventions: {
      notes: "Raw markdown in 'content'. 'content_text' is auto-derived — do not set manually. Use the optional 'folder' parameter in ensure_note to place a note in a subfolder. Use search_notes with an empty query to list all notes in a project.",
      ...
      tasks: "Always provide columnId (not just projectId) when creating a task. Use list_ready_tasks to find work that can start now. Use search_tasks with an empty query to list all tasks. Use update_task with archived: true/false to archive or restore a task.",
      ...
      createProject: "upsert_project without projectId auto-creates 5 default columns. Provide projectId to update an existing project.",
    },
  };
}

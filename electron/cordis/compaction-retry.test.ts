 ...
 * dsh session events (llm/retry, compaction/start|summary|end) → projections.
...
describe("cairnCodingPlugin — context-management bridge (2h)", () => {
  it("maps llm/retry to pi-agent:retry with 1-based attempt", () => {
    ...
    const retry = sent.find((s) => s.payload.kind === "retry");
    expect(retry).toBeTruthy();
    expect(retry!.payload).toMatchObject({ sessionId: "s-retry", kind: "retry", data: { attempt: 1, maxRetries: 5, delayMs: 500, error: "429 rate limited" } });
  });
  ...
  it("falls back to failure.code when there is no message", () => {
    ...
    const retry = sent.find((s) => s.payload.kind === "retry");
    expect(retry!.payload).toMatchObject({ data: { attempt: 3, error: "overloaded" } });
  });
  ...
  it("maps compaction start/summary/end to pi-agent:compact + compact-result", () => {
    ...
    const compactEvents = sent.filter((s) => s.payload.kind === "compact");
    expect(compactEvents.map((e) => (e.payload.data as { status: string }).status)).toEqual(["start", "end"]);
    expect(compactEvents[1].payload.data as Record<string, unknown>).toMatchObject({ auto: true });

    const result = sent.find((s) => s.payload.kind === "compact-result");
    expect(result!.payload).toMatchObject({ sessionId: "s-compact", kind: "compact-result", data: { messageCount: 4, summary: "Condensed the earlier exploration." } });
  });
  ...
  it("does not emit compact-result when the close failed", () => {
    ...
    expect(sent.some((s) => s.payload.kind === "compact" && (s.payload.data as { status: string }).status === "end")).toBe(true);
    expect(sent.some((s) => s.payload.kind === "compact-result")).toBe(false);
  });
  ...
  it("extracts summary text from a block-array summary", () => {
    ...
    fire("compaction/end", {});
    const result = sent.find((s) => s.payload.kind === "compact-result");
    expect((result!.payload.data as { summary: string }).summary).toBe("Part A. Part B.");
  });
  ...
  it("ignores events from a non-matching session", () => {
    ...
    expect(sent.some((s) => s.payload.kind === "retry")).toBe(true);
  });
});

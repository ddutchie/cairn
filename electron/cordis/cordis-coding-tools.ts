...
import PtcNodeRuntime from "@deepseek-ai/dsh-ptc-runtime-node";
...
export async function mountCodingStack(ctx: Context, opts: CodingStackOptions): Promise<() => unknown> {
  ...
  if (role !== "automation-dev") {
    ...
    // Workflow + Ralph (dsh-workflow-ptc engine over the Node PTC runtime,
    // plus the dsh-tool-workflow / dsh-tool-ralph model tools). Since dsh
    // 0.1.7 the engine runs each workflow script in a fresh sandboxed Node
    // process (ELECTRON_RUN_AS_NODE, bootstrap bundled to
    // dist-electron/process.js by compile-electron.js), so the runtime and
    // engine inject the per-turn fs/subprocess/sandbox/sandboxPolicy services
    // and mount here with bash rather than globally. The automation-dev
    // persona has no subprocess service, so it gets no workflow tools.
    //
    ...
    // fan-out orchestration INSIDE a live turn. Complementary axes.
    //
    ...
    // `provider: "spawn"` is Cairn's in-process child route.
    await plug(PtcNodeRuntime);
    await plug(PtcWorkflowEngine, { provider: "spawn", maxConcurrentAgents: 0, maxTotalAgents: 1000, maxItemsPerCall: 4096, syncTimeoutMs: 5000 });
    ...
    void PtcNodeRuntime; void PtcWorkflowEngine;
    void toolWorkflowApply; void toolWorkflowInject; void toolWorkflowName;
    void toolRalphApply; void toolRalphInject; void toolRalphName;
  }

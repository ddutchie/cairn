/**
 * Session prompt guard — store-layer home for the cross-remount agent session
 * prompt bookkeeping (moved from `src/lib/agent-prompt-guard.ts` in cleanup
 * Phase 3 so the terminal-sessions slice no longer imports from `src/lib/`).
 *

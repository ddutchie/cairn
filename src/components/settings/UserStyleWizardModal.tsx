    ...
    >
      ...
      <div className="mt-4 space-y-3 min-h-[16rem]">
        {pasteMode && (
          <div className="space-y-3">
            ...
            <textarea
              className={cn(textareaCls, "min-h-[16rem] font-mono text-[0.714rem]")}
              value={fullGuide}
            ...
            {streaming && (
              <textarea
                className={cn(textareaCls, "min-h-[12rem] font-mono text-[0.714rem]")}
                value={streamingText}
                readOnly
                placeholder="Generating…"
              />
            )}
            {!streaming && cheatsheet.trim() && (
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[0.65rem] font-semibold uppercase tracking-wide text-[var(--text-tertiary)]">Cheat sheet</p>
                  <button
                    type="button"
                    onClick={() => setPreviewMode((v) => !v)}
                    className="text-[0.65rem] rounded-md border border-[var(--border)] px-2 py-1 text-[var(--text-secondary)] hover:border-[var(--accent)] hover:text-[var(--text-primary)] transition-colors cursor-pointer"
                  >
                    {previewMode ? "Edit" : "Preview"}
                  </button>
                </div>
                {previewMode ? (
                  <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-4 min-h-[12rem] max-h-[20rem] overflow-y-auto">
                  ...
                  <textarea
                    ...
                    placeholder="Your generated cheat sheet will appear here — or paste one yourself."
                  />
                )}
              </div>
            )}
            <p className="text-[0.65rem] text-[var(--text-tertiary)] leading-relaxed">
              ...
              You can generate a condensed cheat sheet from it here before saving.
            </p>
          </div>
        )}
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-[var(--border)]">
        <Button
          ...
          onClick={pasteMode ? (streaming ? cancelGeneration : () => setPasteMode(false)) : (streaming ? cancelGeneration : (step === 0 ? onClose : () => setStep((s) => s - 1)))}
        >
          {pasteMode ? (streaming ? "Cancel generation" : "Back to guided setup") : (streaming ? "Cancel generation" : (step === 0 ? "Cancel" : "Back"))}
        </Button>
        <div className="flex items-center gap-2">
          {pasteMode && (

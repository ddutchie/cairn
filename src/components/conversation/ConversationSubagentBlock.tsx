  ...
  return (
    <div className="mt-1 rounded-lg border border-[var(--border)] bg-[var(--surface)] overflow-hidden">
      <button type="button" onClick={() => setExpanded((value) => !value)} className="w-full flex items-center gap-1.5 px-2.5 py-1.5 hover:bg-[var(--surface-2)] transition-colors text-left">
        {subagent.running ? <Spinner size={10} tone="accent" /> : <GitBranch size={10} className="text-[var(--accent)] shrink-0" />}
      ...
      {expanded && (
        <div className="border-t border-[var(--border)] px-2.5 py-2 space-y-2 max-h-96 overflow-y-auto">
          ...
          {!subagent.content && !subagent.messages?.length && subagent.running && <div className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-[var(--surface-2)] border border-[var(--border)] w-fit"><Spinner size={9} tone="accent" /><span className="text-[0.643rem] text-[var(--text-tertiary)]">Working…</span></div>}

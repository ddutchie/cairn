...
export function NoteEditor({ note, onBack }: NoteEditorProps) {
      ...
      try {
        ...
        const result = await new Promise<{ content: string }>((resolve) => {
           const unsub = electron.session.onDone((e) => {
             if (e.sessionId !== "chat-ai-text-action") return;
             unsub(); resolve(e as { content: string });
           });
  ...
  useEffect(() => {
    ...
    const electron = window.electron;
    if (!electron) return;
           const unsub = electron.session.onTool((e) => {
       if (e.sessionId !== "chat-spawn-tasks") return;
       if (e.name === "create_task") {
  ...
  // The ReactMarkdown component-override map (read-mode preview). Extracted to
  // note-markdown-components.tsx; kept referentially stable there via the same
  // useMemo([toggleCheckbox]) + ref-closure pattern so unrelated store updates
  // don't rebuild the map (which would re-parse the preview + remount KaTeX).
  const mdComponents = useNoteMarkdownComponents({ toggleCheckbox, notesRef, previewScrollRef });

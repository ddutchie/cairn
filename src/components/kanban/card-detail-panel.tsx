...
import {
  Tag as TagIcon, FileText, Link, ExternalLink, X,
} from "lucide-react";
        ...
        <div>
          ...
          {isEditingDesc ? (
            <textarea
              ...
              rows={6}
              className="w-full bg-[var(--surface-2)] border border-[var(--border)] rounded-lg px-3 py-2.5 text-sm text-[var(--text-secondary)] placeholder:text-[var(--text-tertiary)] focus:outline-none focus:border-[var(--accent)] resize-none leading-relaxed min-h-[10rem]"
            />
          ) : (
            <div
              onClick={(e) => {
              ...
              className="w-full bg-[var(--surface-2)] border border-[var(--border)] hover:border-[color-mix(in_srgb,var(--accent)_40%,transparent)] rounded-lg px-3 py-2.5 text-sm text-[var(--text-secondary)] min-h-[10rem] cursor-pointer transition-colors overflow-y-auto"
            >

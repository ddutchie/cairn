/**
 * Unit tests for calendar drop-resolution (calendar-dnd.ts).
 *
 * Covers: drop onto a day (set), drop onto "unscheduled" (clear), no-op drops
 * (same day / already unscheduled), and invalid droppable ids.
 */

import { describe, it, expect } from "vitest";
import { resolveDateDrop, UNSCHEDULED_DROP_ID } from "./calendar-dnd";

describe("resolveDateDrop", () => {
  it("sets dueDate when dropped on a day key", () => {
    expect(resolveDateDrop("2026-07-01", { dueDate: undefined })).toEqual({
      dueDate: "2026-07-01",
    });
    expect(resolveDateDrop("2026-07-01", { dueDate: "2026-06-10" })).toEqual({
      dueDate: "2026-07-01",
    });
  });

  it("clears dueDate when dropped on the unscheduled tray", () => {
    expect(resolveDateDrop(UNSCHEDULED_DROP_ID, { dueDate: "2026-06-10" })).toEqual({
      dueDate: undefined,
    });
  });
  ...
  it("returns null for null/empty/invalid droppable ids", () => {

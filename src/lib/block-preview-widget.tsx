 ...
 *
 *  1. Synchronous mount (`flushSync`) so CodeMirror's first height measure —
 *     which positions every line below the widget — sees real content, not a
 *     0px shell.
 *  2. A `ResizeObserver` that calls `view.requestMeasure()` when the height
 *     settles later (async syntax/markdown/image resolution, fonts, toggles).
 *  3. `estimatedHeight = -1` (never assume the widget matches the replaced

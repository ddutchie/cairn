 ...
 *
 * ⚠️ RENDERER COPY. The canonical version lives at `shared/sync/merge3.ts`
 * (used by the Electron main process + mobile). The renderer tsconfig excludes
 * `shared/`, so this identical copy exists so the conflict dialog can run the
 * merge client-side for the live preview. Keep the two in lockstep — the logic
 * is pure and has no dependencies. Both are covered by `shared/sync/merge3.test.ts`.
 *

/**
 * Unified line diff — pure logic for the conflict-resolution dialog.
 *
 * ⚠️ RENDERER COPY. The canonical version lives at `shared/sync/line-diff.ts`
 * (used by mobile, which can import `shared/`). The renderer tsconfig excludes
 * `shared/`, so this identical copy exists so the desktop conflict dialog can
 * compute the diff client-side. Keep the two in lockstep — the logic is pure
 * and dependency-free. Both are covered by `shared/sync/line-diff.test.ts`.
 *

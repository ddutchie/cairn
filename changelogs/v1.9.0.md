...
| `electron/shared/read-tools-pure.ts` | **[NEW]** Pure snapshot-based read tools and project context packs. Implemented note and task description truncation limits inside `executeGetProjectContextPack`. |
...
| `electron/lib/read-tools.ts` | Delegated read operations to `read-tools-pure.ts`. Implemented task description truncation inside `executeSearchTasks`. |
| `electron/mcp-server.test.ts` | Updated `upsert_project` tests to assert the unified return shape. Added unit tests for pinned note and task description truncation behavior. |
...
| `electron/mcp/tools/tasks.ts` | Returned unified `toCard` objects across restore, block, and unblock paths. Implemented description truncation inside standalone `search_tasks` tool. |

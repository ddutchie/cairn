...
**When you ship a MAJOR, user-facing feature, add an entry to `scripts/features.config.js`** (not every changelog line — only headline features worth announcing on launch; fixes and internal work stay in `changelogs/` only). Append at the end in release order with a stable, never-reused `id` (e.g. `v2.6.0-my-feature`), the release `version` (`vX.Y.Z`), a short `title`, a one-word-ish `category`, a `description`, and 3–4 `highlights` (a `"Prefix: rest"` highlight bolds the prefix). Run `node scripts/generate-features.js` after editing; it validates the shape and rejects duplicate ids.

**Gating is per MINOR version, not exact patch**: the modal auto-shows all *unseen* entries whose minor matches the newest minor in the registry. So a headline feature dropped in 2.6.1 still surfaces to a user who already saw 2.6.0, but a user who saw 2.6.0's entries only sees the new 2.6.1 ones (the per-user `seenFeatures` ids prevent re-shows).

**Condensing housekeeping**: one card per minor. When a new minor ships (e.g. `v2.6.0`), condense the previous minor's cards into a single `v2.5.x` entry — stable `id` `"v2.5.x"`, `version` `"v2.5.x"` (the generator accepts the `.x` suffix), a short "Cairn 2.5 — Highlights" title, and one `"Prefix: rest"` highlight per feature. Ancient majors can be merged into a single `v1.x` card covering the whole line. Condensed cards are never auto-shown (they aren't the newest minor); they only appear when the user browses What's New from Settings (forceOpen).
